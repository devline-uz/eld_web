...
## Events that exist
`notification.new` (`user:{id}`: bell dot, panel cache, toast if CRITICAL) · `message.new` (`conversation:{id}`: append, reorder, unread) · `trip.status_changed` (`fleet`: patch row, invalidate KPIs) · `safety.event_created` (`fleet`: invalidate list, Safety badge +1) · `geofence.transition` (`fleet`: flash unit) · `report.ready` (`user:{id}`, from the worker via the Redis bridge B-49 — shell `useReportReadyShell` invalidates reports everywhere and toasts outside `/reports/*`; report screens toast themselves + keep the 3 s poll fallback, WD-094) · `eld.events_ingested` (`vehicle:{id}`: invalidate `logDay`/`logEvents`) · `telemetry.point` (`vehicle:{id}`: Live status card, throttled) · `device.ble_state` (`vehicle:{id}`: BLE chip, Devices table).

**Driver-only (emitted, no web handler):** `log.edit_requested`, `unidentified.confirmation_requested` — both go to `driver:{id}` for the driver app (`DRIVER_ONLY_REALTIME_EVENTS` in `events.ts`). The web may be in that room on HOS Logs / Driver detail, but must not handle them.
**Identity room:** `user:{id}` is auto-joined — `useRoom('user:…')` only attaches listeners and never (un)subscribes (WB-248); shell listeners use `useRealtimeEvent`.
