...
- No vehicle-group model (B-46, remaining half) — IFTA `Vehicle group` stays disabled. The jurisdiction list and Activity grouping were done client-side on 2026-09-25 and are no longer gaps.
- **B-98 *(new, 2026-09-25)* — connectors for Pacific Track, DAT, Geotab, Zapier.** Blocks W-22 Integrations `Connect` on those four catalog cards and in the marketplace modal (`available: false`). The public `eld_backend` has `INTEGRATION_PROVIDERS` = `mcleod`, `wex`, `comdata`, `quickbooks`, `slack`, `webhook`; `PUT /integrations/:provider` answers 422 for any other provider, and there is no OAuth flow and no connector for these four. Needed: each provider added to `INTEGRATION_PROVIDERS` with a working connector (OAuth where the vendor requires it), and `GET /integrations/catalog` returning `available: true` for it — the web then maps the card's `provider` and the button enables with no other change. Zapier could partly be served today by API keys + the Custom webhook, but a real `Connect` needs `zapier` whitelisted and the catalog returning `available: true`. Status: **confirmed missing**; the user chose to wait for the backend.
