...
| B-12 | `POST /support/tickets` should accept `support:READ`, not just `FULL` | W-24 Viewer `New ticket`, 11.22 | **confirmed missing** — `@Perm('support', 'FULL')` on `SupportController#create`; VIEWER carries `support: READ`. The design shows `+ New ticket` for every role (§21.4 documents this as a deliberate READ exception). The button stays in the DOM for VIEWER; since stage 4 (WB-245/WB-246) `Submit ticket` and `Submit feedback` are disabled without `support:FULL`, with the reason on screen, and a server `403 FORBIDDEN` still surfaces inline as a fallback. `POST /feedback` has the same `support:FULL` gate and the same gap for Viewer's `Send feedback` (W-25). |
...
| B-93 *(new)* | `POST /geofences` takes `type: 'CIRCLE' \| 'POLYGON'` only — no address-based shape and no server geocoder | 11.1 Create a geofence — the `Address` shape segment | `type: 'ADDRESS'` with `address` + `radiusMeters`, geocoded server-side into a circle (or a `POST /geocode` the web can call before sending a `CIRCLE`) | **confirmed missing** (zod `geofenceSchema.type`). `Address` used to silently send `POLYGON`; it is now `disabled` with "Address shape is not available yet — the geofence API accepts only circle or polygon shapes." visible under the segments. Rectangle and Polygon both send `POLYGON` and each now shows as pressed on its own. |
