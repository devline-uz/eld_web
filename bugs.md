...
## WB-247 · DVIR report: `Download PDF` shown to VIEWER always 403'd — ✅ fixed for real (B-96 shipped 2026-09-24)
**Found:** 2026-09-24, Phase 13 unlock (web-reports-transfer). **Severity:** medium.
**File:** `src/features/reports/DvirReportPage.tsx` (`Download PDF`), `src/features/reports/IftaReportPage.tsx` (`Download IFTA PDF`). The button called `POST /reports/generate` (`reports` FULL); the READ shortcut `GET /reports/{dvir,ifta,activity}` was CSV-only, so for VIEWER every click was a 403 and both buttons were gated `<Can perm="reports" level="FULL">` (absent for VIEWER). B-96 shipped: the READ shortcuts now take `format=PDF` (confirmed live on :3002, `202 { reportId, status }` with `reports:READ`). Both buttons now call `useExportWhenReady().start({ kind, params: { …, format: 'PDF' } })` — the same READ-level path `Export CSV` uses — and the `Can FULL` wrap is removed, so VIEWER keeps `Download PDF` / `Download IFTA PDF` per W-14. `shared/api/reports.ts` `QueueShortcutInput` gained an optional `format` field. Activity report has no PDF button in the design (§10 W-13 lists only `Export CSV`/`Print`), so it is unchanged.

## WB-248 · Leaving a Reports screen dropped the user's own socket room — bell and `report.ready` went silent — ✅ fixed
**Found:** 2026-09-24, web-architect, while wiring the shell-level `report.ready`. **Severity:** high.
**File:** `src/shared/realtime/useRoom.ts`. `features/reports/useReportJobs.ts` calls `useRoom('user:{id}')`; on unmount `useRoom` emitted `unsubscribe`, and the gateway's `onUnsubscribe` really `leave`s the room (`realtime.gateway.ts:97-99`) — including the `user:{id}` room it auto-joins on connect. After one visit to a report screen, `notification.new` (bell, CRITICAL toast) and `report.ready` stopped until the next reconnect. `useRoom` now treats `user:*` as the identity room: it attaches listeners only, never emits `subscribe`/`unsubscribe`, and reports `joined` = `connected`. Test: `useRoom.test.tsx` ("WB-248 …").

## WB-249 · Bundle budget exceeded on the final gate — ✅ fixed 2026-09-24 (web-architect)
**Found:** 2026-09-24, web-qa-a11y, final testing gate after 9 web agents' backend-unlock wave. **Severity:** medium (build still succeeds; CI's bundle-budget script exits 1).
**Detail:** `npm run build` → `entry + vendor + shell (initial)` = **225.9 KB gzip** against the 220 KB budget (§16.1) — over by 5.9 KB. Every other line passes: route chunks all ≤ 90 KB, `maplibre` 421.0/425.0 KB, `recharts` 96.9/120 KB, total 1060.9/1228.8 KB. Last recorded pass was 201.7 KB (`tasks.md` line 92, pre-dating this wave), so ~24 KB of eager growth landed since, spread across many small diffs to always-eager shared modules (`AuthProvider.tsx`, `Topbar.tsx`, `routePrefetch.ts`, `shared/api/*.ts`, `shared/realtime/events.ts`/`useRoom.ts`) rather than any single new dependency (`package.json` unchanged) or an accidentally-eager route import (`app/router.tsx` still lazy-loads every screen; `stats.html`/sourcemaps regenerated for the next investigator). **Not fixed here** — root-causing which shared module(s) to trim needs a proper per-module size diff (source-map-explorer or the regenerated `stats.html`) that this session's remaining budget did not allow; the gate is correctly left **FAIL**, not weakened. Next step: bisect the eager chunk (`vendor-BBEMMv7r.js` + `index-*.js` + `vendor-react-*.js`) against `stats.html` and split or lazy-load whatever grew.

**Cause (web-architect, sourcemap attribution of `index-*.js` + `vendor-*.js`):** Rollup places a module imported by both the eager shell and lazy chunks into the *eager* chunk, with every export any chunk uses. Three shell imports did exactly that: (1) `app/routePrefetch.ts` statically imported the prefetch query factories from `shared/api/{vehicles,trips,dvir,lookups}` → all four whole modules eager; (2) `Sidebar.tsx` imported `useCarrier` from `settingsAdmin.ts` and `vehiclesCountQuery` from `vehicles.ts` → the whole Settings API module eager; (3) `shared/realtime/reportReady.ts` imported `fetchReport`/`fetchReportDownload` from `reports.ts` → `reports.ts` plus its `drivers.ts` and `hosLogs.ts` imports eager (~31 KB min together). Separately, the catch-all `vendor` manualChunk kept lazy-only Radix popover/dropdown-menu/menu/popper/roving-focus/tabs/checkbox/switch and `@floating-ui/*` in the initial payload (~15 KB gzip).
**Fix:** the prefetch table moved to `app/routeQueries.ts`, reached only via `import()` inside `warmRoute` (now returns a Promise; `routePrefetch.test.ts` awaits it). New small eager modules `shared/api/carrier.ts` (`useCarrier`, `CarrierRow`, `HosRuleset`) and `shared/api/vehicleCounts.ts` (`vehiclesCountQuery`), re-exported from `settingsAdmin.ts` / `vehicles.ts` so feature imports are unchanged; `fetchReport`/`fetchReportDownload` moved into `reportFiles.ts` and re-exported from `reports.ts`. `vite.config.ts` gained a lazy `vendor-popover` bucket. Budget unchanged: initial **225.9 → 203.6 KB** / 220 KB, maplibre 421.0/425, recharts 96.9/120, total 1066.5/1228.8 KB, every route chunk ≤ 90 KB. Rule for the next wave: the shell imports only small single-purpose `shared/api` modules, never a feature's composition module.

## WB-250 · New ticket / Feedback: `Submit` still gated on `support:FULL` for VIEWER even though B-12 shipped `support:READ` server-side — ✅ fixed 2026-09-24
**Found:** 2026-09-24, web-qa-a11y, while reconciling `buttonsAndInputs.md` against `backend-gaps.md`'s Phase 13 handoff. **Severity:** low (VIEWER already had a working fallback path — the button was disabled with a clear reason, not broken — so no compliance or data-loss risk; purely a missed capability).
**File:** `src/features/support/components/NewTicketModal.tsx`, `src/features/support/FeedbackPage.tsx`, `src/features/support/lib/copy.ts`. `backend/tasks.md` Phase 13H marks "B-12 tickets/feedback allowed with `support:READ`" `[x]` shipped 2026-09-24, and `backend-gaps.md`'s Phase 13 handoff lists B-12 among the closed gaps, but `NewTicketModal.tsx`/`FeedbackPage.tsx` still computed `canSubmit = can('support', 'FULL')`. **Fix:** both now compute `canSubmit = can('support', 'READ')`; since the route and nav item are already gated on `support` (NONE never reaches these screens), any role that can open Support/Feedback can now submit. `Start chat` (`StartChatModal.tsx`, B-90) was already ungated and needed no change. `SUPPORT_REASON.ticketForbidden`/`feedbackForbidden` are kept only as the inline-403 fallback text for a genuine server-side refusal. Updated `FeedbackPage.test.tsx`/`SupportPage.test.tsx` READ-role tests to assert Submit is enabled and the request is sent; updated `buttonsAndInputs.md` rows and closed B-12 there.

## WB-251 · Integrations: `Connect` on Custom webhook stores an empty config — nothing is ever delivered — ✅ fixed 2026-09-25
**Found:** 2026-09-25, while checking the W-22 `Connect` rows against the public `eld_backend` (B-98 investigation). **Severity:** medium (the connect succeeds, so the user believes events are being sent; no data loss, no compliance impact).
**File:** `src/features/settings/IntegrationsPage.tsx:88-96` (`handleConnect`). Every `Connect` sends `PUT /integrations/:provider` with `{ enabled: true, config: {} }`. For `webhook` there is no URL and no secret in `config`, so the backend's `WebhooksService.notify` skips the integration and no event is ever posted. **Fix needed (web):** `Connect` on `Custom webhook` must collect the endpoint URL (and a signing secret, if the backend expects one) in a small modal and send them in `config`, instead of connecting blind. Not a backend gap — the endpoint accepts `config`.
**Backend contract (public `eld_backend`):** `UpsertIntegrationDto` = `{ enabled, config: Record<string, unknown> }` with no per-provider validation; `IntegrationsService.upsert` **replaces** the whole config; secret-looking keys are encrypted at rest and `GET` returns them as `[REDACTED]`. `WebhooksService.notify` reads `config.url` (any non-empty string — no scheme check), and `webhook.processor.ts` signs each delivery with `getDecryptedSecret('webhook', 'secret')`, which throws when the secret is missing — so `secret` is effectively required. `POST /integrations/webhook/test` (`{ eventType='test.ping', payload={} }`, `integrations` FULL) returns `409 INTEGRATION_NOT_CONFIGURED` when no enabled webhook has a URL.
**Fix:** new `src/features/settings/components/WebhookConfigModal.tsx` (RHF + zod, `fields.webhookUrl()` http/https, `fields.webhookSecret()` non-blank and untrimmed; show/hide toggle; `Generate` = 32 random bytes hex; 422 paths `config.url`/`config.secret` → field errors; dirty-close). `IntegrationsPage.tsx`: `Connect` on `Custom webhook` (card and marketplace) opens the modal and sends `PUT /integrations/webhook` `{ enabled: true, config: { url, secret } }`; the connected card gains `Configure` (URL prefilled, secret blank and required again because the backend replaces the whole config — the modal says so) and `Send test` (`useSendWebhookTest`, `POST /integrations/webhook/test`), both inside the existing `canFull` gate. A connected webhook with no URL shows `Connected · no endpoint set`. Other providers connect exactly as before. MSW: the `PUT` mock replaces and redacts config like the backend, `DELETE` drops config, the test endpoint 409s without a URL; the seed uses `[REDACTED]`. Tests: `IntegrationsPage.test.tsx` (9 new), `schemas.test.ts` (2 new).
