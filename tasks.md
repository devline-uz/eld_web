...
- [x] WB-048 fan-out: W-13/W-15 on `GET /reports/activity/summary` (one request; W-15 single-driver → one range), TanStack `signal` on every reports query (WD-070/WD-071), `src/app/logsRangeFanOut.test.tsx` caps `/logs/*/range` at 5 per screen (`web-reports-transfer`, 2026-09-15)
...
- [x] 11.23 Filters wired with real filter content on every list screen tz.md §11.23 names — **W-03 Vehicles and W-06 Drivers** (`web-vehicles-drivers`): status/ELD device/make/year/terminal/defects/firmware and status/terminal/violations/exemptions, active-filter chips, Clear all, URL-synced (`fStatus`/`fDevice`/`fMake`/`fYearFrom`/`fYearTo`/`fTerminal`/`fDefects`/`fFirmware` and `fStatus`/`fTerminal`/`fViolations`/`fExempt`), client-side against the already-loaded full list (backend has no server filter params beyond `status`, gaps B-54/B-55). **W-11 Trips** (`web-dispatch-messaging`): STATUS (displayStatus On time/Late/Loading/Delivered/Cancelled/Planned)/Driver/Unit/Home terminal/Depart date range/no-trailer toggle, active-filter chips, Clear all, URL-synced (`fStatus`/`fDriver`/`fUnit`/`fTerminal`/`fDepartFrom`/`fDepartTo`/`fNoTrailer`), client-side against the already-loaded `useTripsList({ limit: 500 })` set (gap B-59). **W-09 DVIR and W-10 Safety done** (`web-dvir-safety`): DVIR groups Type/Severity/Repair status applied to the `DVIRs` tab only (`fType`/`fSeverity`/`fRepair`, WD-062), client-side against `useDvirsList({ limit: 500 })` (gap B-60); Safety groups Event type/Severity (bucketed 1-5 → Critical/Major/Minor, WD-063)/Coaching status (`fType`/`fSeverity`/`fCoaching`), client-side against `useSafetyEventsList({ limit: 500 })` (gap B-61). Filters now on W-03, W-06, W-09, W-10, W-11 — the screens tz.md §11.23 calls for plus Trips. W-12–W-15, W-18–W-21, W-23 have no filter set in the spec. **Superseded 2026-09-16 (`web-api-client`, WD-073):** W-03/W-11/W-09 are now server-paged (`page`/`limit`/`q`/`status` per DTO, one request per page); the client-side groups run against a bounded newest-first window (`FILTER_WINDOW` = 1,000) only while active, never against a fetch-everything set.
- [x] Perf plan items 2 + 4 (`web-api-client`, WD-073): server-side pagination on W-03 Vehicles / W-11 Trips / W-09 DVIR (DVIR loads only the active tab), session-wide reference lookups (`shared/api/lookups.ts`), `client.list` parallel page walk, `STALE.list` 60 s / `STALE.reference` 10 min, sidebar hover/focus prefetch of route chunk + primary query (`app/routePrefetch.ts`, 150 ms debounce), MSW `serverPage()` honouring the DTO params, contract test `tests/contract/list-params.contract.test.ts`. Verified: tsc 0 errors, eslint 0 warnings, 101 unit files / 1057 tests, 5 contract files / 34 tests, build 209.8 KB gzip entry+vendor+shell.
- [x] Perf plan item 3 (`web-dashboard-fleet`, WD-074): W-01 Fleet Dashboard reads one `GET /dashboard/summary` (`shared/api/dashboardSummary.ts`, `cachePolicy('live')`) instead of the 6 requests it used to fire (`/live/fleet`, `/violations`, `/unidentified`, `/notifications`, `/carrier`, 2× `/vehicles?limit=1`); every card's loading/error state still reads off that one query, KPI row/donut/map preview/violations table/carrier subtitle unchanged visually. MSW `mocks/handlers/dashboard.ts` composed from the existing fixtures; `backend/docs/openapi.json` regenerated (`npm run openapi:gen`) to document the new operation. Verified: tsc 0 errors, eslint 0 warnings, `src/features/dashboard` + `src/shared/api` 130/130 tests, `test:contract` 34/34.
- [ ] 11.24 saved views persisted — persistence hook ready: `useUiPreference('savedViews'|'tableColumns', screen, defaults)` in `shared/auth/uiPreferences.ts` (`PUT /me/preferences` + localStorage fallback, tested); screens still have to adopt it
- [x] 11.26 Account menu · 11.27 Notifications panel · 11.28 Command palette (⌘K) — built as lazy chunks (WD-054…056, WD-059); 11.28 entity search now one live `GET /search` call, fallback removed (WD-093); 11.27 single read / category segments / human body / click-through live (B-56…B-58 shipped); shell `report.ready` toast + reports invalidation (WD-094); identity-room leak fixed (WB-248)
...
- [x] Bundle budgets met (initial 203.6/220 KB after WB-249; MapLibre line raised to 290 KB with measurement, WD-023)
...
- [x] Backend Phase 13 adoption (`web-api-client`, 2026-09-24, WD-088): `KNOWN_GAPS` / `ACCEPTED_BUT_UNDOCUMENTED` emptied; B-68 `resolutionType` migration (`useResolveDefect`, `ResolveDefectModal`, tests); ~20 new endpoint keys, typed hooks + MSW (`mocks/handlers/phase13.ts`) for every Phase 13 endpoint; `dataTransfer` 23rd permission key; 11 new error codes; `tests/contract/phase13.contract.test.ts` checks responses AND request payloads against openapi.json; screen handoff table in `backend-gaps.md`. Verified: tsc 0 errors, eslint 0 warnings, 136 unit files / 1656 tests (shared/format + permissions 100%), `test:contract` 6 files / 86 tests
- [x] Phase 13 handoff consumed on W-04/W-05/W-06/W-07 (`web-vehicles-drivers`, 2026-09-24): W-05 route replay now plays for real (SVG polyline + animated marker over `useVehicleHistories` segments' `lat`/`lon`, 1×/2×/4× speed, scrub slider, row↔map hover, B-4 closed); W-04 Live status reads `useVehicleTelemetry(id, {limit:1})`, ELD device card reads `useDevicesForVehicle` (B-35), Co-driver row reads `useCoDriverPairings` (B-7, also wired on W-07); `AssignDriverModal` `notify` now sent (B-74); `AddDriverModal` `Send invitation now` and `ImportDriversModal`/`ImportVehiclesModal` option fieldsets are real, enabled controls (B-82, B-69); W-06/W-07 show the real `● Email not verified` badge off `Driver.emailVerifiedAt` with a `Resend verification email` row action (B-29–B-31); `Reset app password` calls `POST /drivers/:id/reset-password` on both W-06 and W-07 (B-81); Vehicles bulk `Set inactive` calls `PATCH /vehicles/bulk-status` instead of the per-id `PATCH` fan-out (B-71); W-07 `Documents` tab is now live (`DriverDocumentsTab.tsx`, B-94) — lists `GET /drivers/:id/documents`, uploads via metadata POST → presigned PUT → refetch, deletes with a `ConfirmDelete`, upload card and delete action gated `<Can perm="drivers" level="FULL">`. Verified: tsc 0 errors, `eslint src/features/vehicles src/features/drivers --max-warnings=0` 0 errors/warnings, scoped `vitest run src/features/vehicles src/features/drivers` (single fork) green
