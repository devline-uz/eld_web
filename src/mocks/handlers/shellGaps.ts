// MSW for the topbar overlays (11.27, 11.28). Real endpoints get richer data here; gap endpoints
// are served in the shape recorded in web/backend-gaps.md:
//   ⛔ B-10 GET /search?q=               ⛔ B-56 POST /notifications/:id/read
//   ⛔ B-57 `category` + `counts` on GET /notifications
...
function seedNotifications(): NotificationItem[] {
  return [
    { id: 'ntf_1', type: 'hos_violation', category: 'VIOLATIONS', title: 'HOS violation', body: 'John Smith exceeded the 11-hour driving limit by 00:26', objectType: 'Driver', objectId: 'drv_1', readAt: null, createdAt: ago(2 * MIN) },
    { id: 'ntf_2', type: 'eld_disconnected', category: 'VIOLATIONS', title: 'ELD disconnected', body: 'Unit #110 · PT30_77D2 has been offline for 46 minutes', objectType: 'Vehicle', objectId: 'veh_110', readAt: null, createdAt: ago(45 * MIN) },
    { id: 'ntf_3', type: 'break_due', category: 'VIOLATIONS', title: 'Break due soon', body: 'Marvin McKinney needs a 30-minute break in 00:18', objectType: 'Driver', objectId: 'drv_7', readAt: null, createdAt: ago(60 * MIN) },
    { id: 'ntf_4', type: 'maintenance_overdue', category: 'MAINTENANCE', title: 'Maintenance overdue', body: 'Unit #104 oil & filter service was due 2 days ago', objectType: 'WorkOrder', objectId: 'wo_1', readAt: null, createdAt: ago(180 * MIN) },
    { id: 'ntf_5', type: 'unassigned_driving', category: null, title: 'Unassigned driving', body: '1h 12m of driving with no driver logged in on unit #103', objectType: 'UnidentifiedSegment', objectId: 'uds_1', readAt: ago(200 * MIN), createdAt: ago(300 * MIN) },
    { id: 'ntf_6', type: 'report_ready', category: null, title: 'Report ready', body: 'FMCSA audit pack for Jun 2025 finished generating', objectType: 'Report', objectId: 'rpt_1', readAt: ago(600 * MIN), createdAt: ago(30 * 60 * MIN) },
  ];
}
