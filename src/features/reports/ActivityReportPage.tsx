...
// Backend D-107 — `Group by vehicle group` (`?groupBy=vehicleGroup`) asks the same endpoint for one
// row per group of each driver's CURRENT unit; KPIs, paging and filters are unchanged.
...
import {
  useActivityGroupSummary,
  useActivitySummary,
  useTransferConfig,
  useReportDrivers,
  type ActivityGroupBy,
  type ActivitySummaryGroupItem,
...
const UNGROUPED_ROW_ID = '__ungrouped';
...
const GROUP_BY_OPTIONS: { value: ActivityGroupBy; label: string }[] = [
  ...
  { value: 'vehicleGroup', label: 'Group by vehicle group' },
];

/** Grouping follows each driver's current unit (backend D-107): say so under the toolbar. */
export const GROUP_BY_UNIT_NOTE =
  'Drivers are grouped by the vehicle group of the unit they are assigned to now. A driver who changed units during this period counts toward the current unit’s group.';

/** Off · SB · Driving · On · Distance · Violations · Certified — identical for driver and group rows. */
type DutyTotals = Pick<
  ActivitySummaryItem,
  'days' | 'offSec' | 'sbSec' | 'drivingSec' | 'onSec' | 'distanceMi' | 'violations' | 'certifiedDays'
>;

function dutyColumns<T extends DutyTotals>(): ColumnDef<T, unknown>[] {
  return [
...
export default function ActivityReportPage() {
  ...
  const groupBy: ActivityGroupBy = params.get('groupBy') === 'vehicleGroup' ? 'vehicleGroup' : 'driver';
  const byGroup = groupBy === 'vehicleGroup';
  ...
  const summaryParams = { from, to, page, limit, sort: SORT, status: 'ACTIVE' as const, ...(terminal ? { terminal } : {}) };
  const driverSummary = useActivitySummary(summaryParams, !byGroup);
  const groupSummary = useActivityGroupSummary(summaryParams, byGroup);
  // The active query drives KPIs, paging and states; only the table rows differ by mode.
  const summary = byGroup ? groupSummary : driverSummary;
  const error = summary.error as ApiError | null;
  const groups = useVehicleGroups();
  const colorOf = useMemo(() => new Map((groups.data ?? []).map((g) => [g.id, g.color])), [groups.data]);
  ...
  const rows = driverSummary.data?.items ?? [];
  const groupRows = groupSummary.data?.items ?? [];
  const shown = byGroup ? groupRows.length : rows.length;
  const itemLabel = byGroup ? 'groups' : 'drivers';
  ...
  useDynamicSubtitle(`${rangeLabel(from, to)} · ${formatNumber(total)} ${itemLabel} · duty totals and distance`);
  ...
  const columns = useMemo<ColumnDef<ActivitySummaryItem, unknown>[]>(
    () => [
      ...
      ...dutyColumns<ActivitySummaryItem>(),
      {
        id: 'open',
        header: '',
        cell: ({ row }) => (
  ...
  const groupColumns = useMemo<ColumnDef<ActivitySummaryGroupItem, unknown>[]>(
    () => [
      {
        id: 'group',
        header: 'Vehicle group',
        cell: ({ row }) => {
          const { groupId, name } = row.original;
          const color = groupId ? colorOf.get(groupId) : null;
          return (
            <span className="flex items-center gap-2">
              <span
                aria-hidden
                className={cn('size-2.5 shrink-0 rounded-full', !color && 'bg-border')}
                style={color ? { backgroundColor: color } : undefined}
              />
              <span className={cn('font-medium', groupId ? 'text-text' : 'text-text-muted')}>{name}</span>
            </span>
          );
        },
      },
      { id: 'drivers', header: 'Drivers', meta: { numeric: true }, cell: ({ row }) => <Num>{formatNumber(row.original.drivers)}</Num> },
      ...dutyColumns<ActivitySummaryGroupItem>(),
    ],
    [colorOf],
  );
    ...
    <div className="flex flex-col gap-card-gap">
      <div className="flex flex-wrap items-center gap-2 print:hidden">
        ...
        <SelectMenu
          ...
          onSelect={(value) => {
            setPage(1);
            setParam('groupBy', value === 'vehicleGroup' ? value : null);
          }}
        />
      ...
      {byGroup && <p className="text-caption text-text-muted print:hidden">{GROUP_BY_UNIT_NOTE}</p>}
      ...
      <Card padded={false}>
        <div className="p-card">
          <SectionHeader
            title={byGroup ? 'Duty totals by vehicle group' : 'Duty totals by driver'}
            subtitle="Totals are calculated from certified and uncertified logs"
            action={
              !summary.isLoading && !summary.isError ? (
                <Badge tone="neutral" className="tabular-nums">{`${formatNumber(total)} ${itemLabel} · showing ${shown}`}</Badge>
              ) : undefined
            }
          />
        </div>
        {summary.isError ? (
          ...
          <>
            {byGroup ? (
              <DataTable
                caption="Duty totals by vehicle group"
                data={groupRows}
                columns={groupColumns}
                getRowId={(r) => r.groupId ?? UNGROUPED_ROW_ID}
