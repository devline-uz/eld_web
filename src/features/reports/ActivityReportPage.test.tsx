...
import ActivityReportPage, { GROUP_BY_UNIT_NOTE } from './ActivityReportPage';
...
describe('W-13 Reports · Activity report', () => {
  ...
  it('Group by vehicle group asks for groupBy=vehicleGroup and draws one row per group (D-107)', async () => {
    const requests: Record<string, string>[] = [];
    server.use(
      http.get(url(endpoints.reports.activitySummary), ({ request }) => {
        const search = new URL(request.url).searchParams;
    ...
    await userEvent.click(await screen.findByRole('menuitem', { name: 'Group by vehicle group' }));

    await screen.findByRole('row', { name: /Midwest linehaul/ });
    expect(screen.getByRole('row', { name: /Ungrouped/ })).toBeInTheDocument();
    expect(requests.at(-1)).toMatchObject({ groupBy: 'vehicleGroup', page: '1' });
    expect(screen.getByRole('heading', { name: 'Duty totals by vehicle group' })).toBeInTheDocument();
    expect(screen.getByText('2 groups · showing 2')).toBeInTheDocument();
    expect(screen.getByText(GROUP_BY_UNIT_NOTE)).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Open logs/ })).toBeNull();
    expect(screen.getByTestId('location').textContent).toContain('groupBy=vehicleGroup');
  });
});
